		...
		const fetchItems = async () => {
			...
			try {
				...
				if (data.results.length !== 0) {
					setItems((prevItems) => [...prevItems, ...(data.results.filter((p: Item) => !prevItems.some((prev) => prev.id === p.id)) as Item[])]);
				} else {
					setNoMore(true);
				}
			} catch (error) {
	...
	return (
		<div className="min-h-screen bg-[#313338] text-white">
			...
			<div className="container mx-auto px-6 py-8">
				...
				{!noMore && loading && <p className="text-center mt-4">{loadingText}</p>}
				{!loading && items?.length === 0 && <p className="text-center mt-4">{notFoundText}</p>}
			</div>
		</div>
	);
}
